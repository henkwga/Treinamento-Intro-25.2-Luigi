...
function StatusBadge({ s }: { s: Compra["status"] }) {
  const MAP: Record<
    Compra["status"],
...
export default function OrdersPage() {
  ...
  async function load() {
    setLoading(true);
    const r = await fetch("/api/purchases/me", {
      cache: "no-store",
      credentials: "include",
    });
                  ...
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div className="space-y-0.5">
                      <div className="text-sm text-white/80">
                        Pedido{" "}
                        <span className="font-semibold">#{c.id.slice(0, 8)}</span>
                      </div>
                    ...
                    {c.itens.map((it) => (
                      <div key={it.id} className="flex items-center gap-3 py-3">
                        <div className="relative h-12 w-12 overflow-hidden rounded">
                          <Image
                            src={it.produto.cover || "/icons/under-construction.png"}
                            alt={it.produto.nome}
                            fill
                            sizes="48px"
