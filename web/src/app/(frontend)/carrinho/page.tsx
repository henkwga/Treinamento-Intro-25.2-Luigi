...
export default function CartPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`/api/products?ids=${encodeURIComponent(ids.join(","))}`, { cache: "no-store" })
      .then(r => r.json())
    ...
    <>
      ...
      <main className="min-h-screen bg-[#0e0f11] pt-20 text-white">
        <div className="mx-auto max-w-5xl px-4 pb-16">
          ...
          {!items.length ? (
            ...
            <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
              <div className="space-y-3">
                {items.map((it) => (
                  <div key={it.id} className="flex gap-4 rounded-xl border border-white/10 bg-[#181818]/60 p-3">
                    <img src={it.cover} alt={it.title} className="h-24 w-24 flex-none rounded-lg object-cover" />
                    <div className="min-w-0 flex-1">
                      <h3 className="truncate text-sm font-semibold">{it.title}</h3>
                      <div className="mt-3 flex items-center gap-2">
                        <button onClick={() => dec(it.id)} className="h-8 w-8 rounded-full border border-white/10 bg-white/10 text-sm">−</button>
                        <input
                          type="number"
                          min={1}
                          value={it.qty}
                        ...
                        />
                        <button onClick={() => inc(it.id)} className="h-8 w-8 rounded-full border border-white/10 bg-white/10 text-sm">+</button>
                      </div>
                    </div>
                    <div className="flex flex-col items-end justify-between">
                      ...
                      <button onClick={() => removeLine(it.id)} className="text-xs text-red-400 hover:underline">
                        Remover
                      </button>
                    </div>
                  </div>
              ...
              <aside className="h-max rounded-2xl border border-white/10 bg-white/5 p-5">
                ...
                <button onClick={checkout} disabled={!items.length} className="mt-2 w-full rounded-full bg-[#ffd100] px-4 py-2 text-sm font-semibold text-black disabled:opacity-60">
                  Finalizar compra
                </button>

                <button onClick={clearCart} className="mt-3 w-full rounded-full border border-white/10 bg-white/10 px-4 py-2 text-sm">
                  Esvaziar carrinho
                </button>
          ...
          {loading && <p className="mt-4 text-center text-white/60">Atualizando preços…</p>}
        </div>
      </main>
    </>
  );
